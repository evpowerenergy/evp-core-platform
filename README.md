...
**Frontend (หลัก):**
- **Google Cloud Run** — Docker image (Node build + nginx) จาก `Dockerfile` และ `cloudbuild.yaml`
  - รายละเอียด: [`docs/CLOUD_DEPLOY.md`](./docs/CLOUD_DEPLOY.md), [`docs/setup/GCP_CLOUD_RUN_SUMMARY.md`](./docs/setup/GCP_CLOUD_RUN_SUMMARY.md)

**Frontend (สำรอง / โปรเจกต์เก่า):**
- **Vercel** — ยังมี `vercel.json` ใน repo; ใช้เป็นทางเลือกสำรอง ไม่ใช่ช่องทาง production หลัก
...
- **Server (ฝั่งที่รัน serverless เท่านั้น เช่น Vercel สำรอง):** `SUPABASE_SERVICE_ROLE_KEY` สำหรับ keep-alive / API
...
**Frontend หลัก — Google Cloud Run:** ใช้ `Dockerfile` + `cloudbuild.yaml` สร้าง image แล้ว deploy ไป Cloud Run (พอร์ต 8080, nginx serve `dist/`). ขั้นตอนเต็มและ checklist อยู่ที่ **[`docs/CLOUD_DEPLOY.md`](./docs/CLOUD_DEPLOY.md)** และ **[`docs/setup/GCP_CLOUD_RUN_DEPLOY.md`](./docs/setup/GCP_CLOUD_RUN_DEPLOY.md)**

**Frontend สำรอง — Vercel (โปรเจกต์เก่า):** ถ้ายังเปิดใช้ — เชื่อม repo, ตั้ง env ใน Vercel Dashboard, deploy ตามเดิม
...
ดูรายละเอียดเพิ่มเติม: [`docs/CLOUD_DEPLOY.md`](./docs/CLOUD_DEPLOY.md), [`docs/setup/DEPLOY_INSTRUCTIONS.md`](./docs/setup/DEPLOY_INSTRUCTIONS.md)
...
- **Cloud Deploy (GCP หลัก + สรุป Vercel สำรอง)**: [`docs/CLOUD_DEPLOY.md`](./docs/CLOUD_DEPLOY.md)
- **Deployment Guide (รายละเอียดเฉพาะงาน)**: [`docs/setup/DEPLOY_INSTRUCTIONS.md`](./docs/setup/DEPLOY_INSTRUCTIONS.md)
