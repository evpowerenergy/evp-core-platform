-- Fix safe_insert_lead_with_duplicate_check function
-- แก้ไข created_by จาก INTEGER เป็น UUID

-- ตรวจสอบว่า function มีอยู่หรือไม่
SELECT EXISTS (
  SELECT 1 
  FROM pg_proc 
  WHERE proname = 'safe_insert_lead_with_duplicate_check'
) AS function_exists;

-- แก้ไข function (รัน migration ใหม่)
-- ไฟล์: supabase/migrations/20250130000000_add_safe_insert_lead_function.sql
