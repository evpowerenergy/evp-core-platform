...
import { Store, TrendingUp, Target, Users, Calendar, MapPin, Zap, FileText, UserX, Phone } from "lucide-react";
...
const WholesaleDashboard = () => {
  const { data: salesTeamData } = useFilteredSalesTeamData('sale_wholesale');
  ...
  const [dashboardData, setDashboardData] = useState<WholesaleDashboardData>({
    totalProjects: 0,
    opportunityValue: 0,
    winValue: 0,
    winRate: 0,
  ...
  const [metricModalState, setMetricModalState] = useState({
    ...
    title: '',
    leads: [] as any[],
    metricType: 'quotation' as 'quotation' | 'contactable' | 'uncontactable'
  });
  ...
  const { data: leadsFromEdgeFunction, isLoading: leadsFromEdgeFunctionLoading } = useQuery({
    queryKey: ['all-leads-for-wholesale-dashboard', dateRangeFilter],
    queryFn: async () => {
      ...
      // Filter เฉพาะ Wholesale/Wholesales category
      return (result.data || []).filter((lead: any) => 
  ...
  const getLeadsForMetric = async (metricType: 'quotation' | 'contactable' | 'uncontactable') => {
    try {
      ...
      // Only apply date filter if dates are selected (ใช้ created_at_thai เพื่อดูลีดที่สร้างในช่วงนั้น)
      ...
      if (metricType === 'quotation') {
        ...
        // ดึงข้อมูลการออก QT สำหรับ Wholesale (รวมทั้ง Wholesale และ Wholesales) - ใช้ระบบใหม่ที่ใช้ view lead_qt_itemized
        const quotationDataWholesale = await getQuotationDataFromView(
        ...
        const quotationDataWholesales = await getQuotationDataFromView(
          startDate || new Date().toISOString(),
          endDate || new Date().toISOString(),
          'Wholesales',
          salesFilterParam
        ...
        // รวมข้อมูลจากทั้งสอง category - ใช้ข้อมูลจาก view ที่ทำ deduplication แล้ว
        const allQuotationLeads = [...quotationDataWholesale.quotationLeads, ...quotationDataWholesales.quotationLeads];
        
        // ✅ รวมข้อมูลตาม lead (ไม่ซ้ำ) และรวม QT จากทั้งสอง category
        const leadQuotationMap = new Map();
        allQuotationLeads.forEach(lead => {
          const leadId = lead.leadId;
          if (leadQuotationMap.has(leadId)) {
            // รวมยอด QT ของ lead เดียวกัน (จากทั้งสอง category)
            const existing = leadQuotationMap.get(leadId);
            existing.totalQuotationAmount += lead.totalQuotationAmount;
            existing.totalQuotationCount += lead.totalQuotationCount;
            existing.quotationNumbers.push(...lead.quotationNumbers);
            existing.quotationItems.push(...lead.quotationItems);
          } else {
            // หา lead data สำหรับ lead นี้
            const leadData = validLeads.find(l => l.id === leadId);
            ...
            if (!leadData) {
              ...
              leadQuotationMap.set(leadId, {
                ...
                category: lead.category || 'Wholesale',
                ...
                quotationItems: [...lead.quotationItems],
                quotationLogs: [{
        ...
        return Array.from(leadQuotationMap.values());
  ...
  // Handle metric click
  const handleMetricClick = async (metricType: 'quotation' | 'contactable' | 'uncontactable') => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [
        ...
        wholesaleSalesData,
        wholesalesSalesData,
        quotationDataWholesale,
        quotationDataWholesales
      ] = await Promise.all([
        newLeadsCountQuery,
        activityLeadsQuery,
        adminLeadsCountQuery,
        salesLeadsCountQuery,
        // ดึงข้อมูลยอดขายที่ปิดการขายสำเร็จ (กรองตาม category Wholesale)
        startDate && endDate ? getSalesDataByCategory(startDate, endDate, 'Wholesale', salesFilterParam) : Promise.resolve({ quotations: [], salesLogs: [], totalSalesValue: 0, salesCount: 0, uniqueLeadIds: [], salesLeads: [] }),
        startDate && endDate ? getSalesDataByCategory(startDate, endDate, 'Wholesales', salesFilterParam) : Promise.resolve({ quotations: [], salesLogs: [], totalSalesValue: 0, salesCount: 0, uniqueLeadIds: [], salesLeads: [] }),
        // ดึงข้อมูลการออก QT (ใช้ระบบใหม่ที่ใช้ view lead_qt_itemized)
        getQuotationDataFromView(startDate || new Date().toISOString(), endDate || new Date().toISOString(), 'Wholesale', salesFilterParam),
        getQuotationDataFromView(startDate || new Date().toISOString(), endDate || new Date().toISOString(), 'Wholesales', salesFilterParam)
      ]);
      ...
      const allQuotations = [...wholesaleSalesData.quotations, ...wholesalesSalesData.quotations];
      const productivityLogsData = [...wholesaleSalesData.salesLogs, ...wholesalesSalesData.salesLogs];
      
      // รวมข้อมูลจากทั้งสอง category
      const quotationData = {
        quotationCount: (quotationDataWholesale?.quotationCount || 0) + (quotationDataWholesales?.quotationCount || 0),
        quotations: [...(quotationDataWholesale?.quotations || []), ...(quotationDataWholesales?.quotations || [])],
        quotationLogsWithQuotations: [...(quotationDataWholesale?.quotationLogsWithQuotations || []), ...(quotationDataWholesales?.quotationLogsWithQuotations || [])]
      };
      
      // ดึงข้อมูลโอกาสการขายสำหรับ Wholesale - ใช้ระบบใหม่ที่ใช้ view
      const opportunityDataWholesale = await getOpportunityDataFromView(
        startDate || new Date().toISOString(),
        endDate || new Date().toISOString(),
        'Wholesale', // filter category เป็น Wholesale
        salesFilterParam
      );
      
      const opportunityDataWholesales = await getOpportunityDataFromView(
        startDate || new Date().toISOString(),
        endDate || new Date().toISOString(),
        'Wholesales', // filter category เป็น Wholesales
        salesFilterParam
      );
      
      // รวมข้อมูลโอกาสการขายจากทั้งสอง category
      const opportunityData = {
        totalOpportunityValue: opportunityDataWholesale.totalOpportunityValue + opportunityDataWholesales.totalOpportunityValue,
        opportunityCount: opportunityDataWholesale.opportunityCount + opportunityDataWholesales.opportunityCount,
        opportunityLogsWithQuotations: [...opportunityDataWholesale.opportunityLogsWithQuotations, ...opportunityDataWholesales.opportunityLogsWithQuotations]
      };
      ...
      const winValue = (wholesaleSalesData.totalSalesValue || 0) + (wholesalesSalesData.totalSalesValue || 0);
      ...
      const totalLeadsFromEdgeFunction = leadsFromEdgeFunction?.length || 0;
      
      // นับจำนวนลีดที่ปิดการขายจริงๆ (ไม่ใช่ QT) จาก salesLeads
      const closedLeadIds = new Set();
      if (wholesaleSalesData.salesLeads) {
        wholesaleSalesData.salesLeads.forEach((lead: any) => {
          closedLeadIds.add(lead.leadId);
        });
      }
      if (wholesalesSalesData.salesLeads) {
        wholesalesSalesData.salesLeads.forEach((lead: any) => {
          closedLeadIds.add(lead.leadId);
        });
      }
      const actualClosedLeadsCount = closedLeadIds.size;
      
      const winRate = totalLeadsFromEdgeFunction > 0 
      ...
      // Get follow-up count from productivity logs
      ...
      // Calculate contactable rate from productivity logs - Count unique leads
      ...
      if (validActivityLeads.length > 0) {
        ...
        // Apply date filter for contact status to show activity within selected date range
        if (startDate && endDate) {
    ...
    <div className="min-h-screen bg-gray-50 p-2">
      <div className="w-full space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">
                Dashboard Wholesale - {selectedSalesName}
              </h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2 min-h-[120px]">
          <Card className="bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-semibold text-blue-800">จำนวนลีดที่รับมา</CardTitle>
              <Target className="h-5 w-5 text-blue-600" />
            </CardHeader>
            <CardContent className="py-3">
              <div className="text-2xl font-bold text-blue-900">{dashboardData.totalProjects.toLocaleString()}</div>
              {/* <p className="text-sm font-medium text-blue-700 mt-1">Projects</p> */}
          ...
          <Card className="bg-gradient-to-r from-green-50 to-emerald-50 border-green-200">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-semibold text-green-800">โอกาส (Opportunity)</CardTitle>
              <TrendingUp className="h-5 w-5 text-green-600" />
            </CardHeader>
            <CardContent className="py-3">
              <div className="text-2xl font-bold text-green-900">฿{dashboardData.opportunityValue.toLocaleString()}</div>
              <p className="text-sm font-medium text-green-700 mt-1">Total Value</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2 mt-2 min-w-0 min-h-[600px]">
          ...
          {/* Card เมตริกเพิ่มเติม - MOVED TO REPLACE Source */}
          <Card>
            <CardHeader className="pb-1">
          ...
          {chartLoading ? (
          ...
          ) : saleChanceData && saleChanceData.length > 0 ? (
            <Card className="shadow-lg border-0 transition-transform hover:scale-[1.02] hover:shadow-xl bg-white min-w-0">
              <CardHeader className="pb-1">
                <CardTitle className="text-sm">โอกาสการขาย</CardTitle>
              </CardHeader>
            ...
            <Card className="shadow-lg border-0 transition-transform hover:scale-[1.02] hover:shadow-xl bg-white min-w-0">
              ...
              <CardContent className="py-6 flex items-center justify-center h-72 min-w-0">
                <div className="text-center text-gray-500 min-w-0">
                  <p>ไม่มีข้อมูลโอกาสการขาย</p>
                  <p className="text-xs mt-1">ในช่วงเวลาที่เลือก</p>
                </div>
              </CardContent>
            </Card>
          )}

          {/* Card จังหวัด (Top 5) - MOVED HERE */}
          <Card className="shadow-lg border-0 transition-transform hover:scale-[1.02] hover:shadow-xl bg-white">
            <CardHeader className="pb-1">
