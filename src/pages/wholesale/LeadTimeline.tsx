...
const WholesaleLeadTimeline = () => {
  ...
  const { data: timeline = [], isLoading: timelineLoading, error: timelineError } = useQuery({
    ...
    queryFn: async () => {
      ...
      // ดึงข้อมูล quotations และ quotation_documents แยกกัน
      if (data && data.length > 0) {
        ...
        const [quotationsData, quotationDocumentsData, salesTeamResult] = await Promise.all([
          supabase
            .from('quotations')
            .select('*')
        ...
        const enrichedData = data.map(log => ({
          ...log,
          quotations: quotationsData.data?.filter(q => q.productivity_log_id === log.id) || [],
          quotation_documents: quotationDocumentsData.data?.filter(qd => qd.productivity_log_id === log.id) || [],
  ...
  if (!isValidId) {
    return (
      <div className="text-center py-12">
        ...
        <Button onClick={() => navigate('/wholesale/my-leads')} className="mt-4">
    ...
    <div className="space-y-6 p-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Button
            variant="outline"
            onClick={() => navigate('/wholesale/my-leads')}
            size="sm"
          >
            <ArrowLeft className="h-4 w-4 mr-2" />
            กลับ
        ...
        <div className="flex gap-2">
          ...
          <Button
            onClick={() => navigate(`/wholesale/productivity-log/add/${leadId}`)}
      ...
      <TimelineList 
        timeline={timeline} 
        onAddLog={() => navigate(`/wholesale/productivity-log/add/${leadId}`)}
...
export default WholesaleLeadTimeline;
