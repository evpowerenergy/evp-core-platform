-- =============================================================================
-- Migration: แก้ไข function create_lead_from_customer_service() ให้ตั้ง sale_owner_id
-- =============================================================================
-- 
-- ปัญหาที่พบ:
-- - เมื่อสร้างลีดใหม่จาก sale follow-up และเลือกผู้รับผิดชอบแล้ว
-- - ระบบตั้ง sale_owner_id = NULL แม้มี sale_follow_up_assigned_to
-- - ทำให้ลีดไปอยู่ใน "ลีดกองกลาง" (sale_owner_id IS NULL) แม้จะไปอยู่ใน "ลีดของฉัน" ด้วย
--
-- การแก้ไข:
-- - เมื่อสร้างลีดใหม่และมี sale_follow_up_assigned_to ให้ตั้ง sale_owner_id = sale_follow_up_assigned_to
-- - เพื่อให้ลีดไปอยู่ใน "ลีดของฉัน" ของผู้รับผิดชอบเท่านั้น ไม่ไปอยู่ใน "ลีดกองกลาง"
-- =============================================================================
...
BEGIN
  ...
  ELSE
    ...
    -- ✅ แก้ไข: ถ้ามี sale_follow_up_assigned_to ให้ตั้ง sale_owner_id = sale_follow_up_assigned_to
    -- เพื่อให้ลีดไปอยู่ใน "ลีดของฉัน" ของผู้รับผิดชอบเท่านั้น ไม่ไปอยู่ใน "ลีดกองกลาง"
    -- ถ้าไม่มี sale_follow_up_assigned_to ให้ sale_owner_id = NULL (ลีดกองกลาง)
    INSERT INTO public.leads (
      ...
      'ติดตามหลังการขาย',
      'รอรับ',
      v_sale_follow_up_assigned_to,  -- ✅ แก้ไข: ใช้ sale_follow_up_assigned_to (ถ้าเป็น NULL จะเป็น NULL)
      v_sale_follow_up_assigned_to,  -- post_sales_owner_id = Sale ที่ติดตาม
      v_combined_notes,
      cs_id,
      NOW(),
