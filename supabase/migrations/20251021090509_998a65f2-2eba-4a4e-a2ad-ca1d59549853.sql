-- Add customer_service_id reference to leads table
ALTER TABLE public.leads 
ADD COLUMN IF NOT EXISTS customer_service_id INTEGER;

-- Create function to create/update lead from customer_services
...
DROP TRIGGER IF EXISTS trigger_create_lead_on_sale_follow_up_complete ON public.customer_services;

-- Create trigger on customer_services
CREATE TRIGGER trigger_create_lead_on_sale_follow_up_complete
  AFTER UPDATE ON public.customer_services
  FOR EACH ROW
  WHEN (
    NEW.sale_follow_up_status = 'completed' 
    AND (OLD.sale_follow_up_status IS NULL OR OLD.sale_follow_up_status != 'completed')
  )
  EXECUTE FUNCTION create_lead_from_customer_service_trigger();